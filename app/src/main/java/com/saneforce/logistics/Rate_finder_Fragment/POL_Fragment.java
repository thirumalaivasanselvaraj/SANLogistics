...
public class POL_Fragment extends Fragment implements View.OnClickListener {
    ...
    String fragname, selected_value, countrynamestring;
    POL_Adapter makerAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.pol_fragment, container, false);
        Declaration_View(v);
        return v;
    }
    ...
    private void Declaration_View(View v) {
        ...
        if (getArguments() != null) {
            ...
            countrynamestring = getArguments().getString("country_name");
    ...
    public void GetDataFromPref() {
        ...
        System.out.println("SELECTED_VALUES" + countrynamestring);
        if (!(Common_Class.isRatefinderEmpty(PortPojoList))) {
            for (int i = 0; i < PortPojoList.size(); i++) {

                if ((PortPojoList.get(i).getCountryName())
                        .equals(countrynamestring)) {
                    if (!maker_list.contains(PortPojoList.get(i).getPOL())) {
                        maker_list.add(PortPojoList.get(i).getPOL());
                    }
        ...
        if (maker_list != null && maker_list.size() > 0) {
            ...
            makerAdapter = new POL_Adapter(maker_list, getActivity(), "POL", selected_value);
            mRecyclerview.setLayoutManager(recylerViewLayoutManager);
            mRecyclerview.setAdapter(makerAdapter);


        } else {
            LisitngMessage(R.drawable.ic_empty_location, "Poll Is Not Available");
