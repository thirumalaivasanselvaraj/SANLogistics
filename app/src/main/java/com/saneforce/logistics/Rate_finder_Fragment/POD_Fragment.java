...
import com.saneforce.logistics.Adapter_Class.POD_Adapter;
...
public class POD_Fragment extends Fragment implements View.OnClickListener {
    ...
    String fragname, selected_value, Select_Poll;
    POD_Adapter makerAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.pod_fragment, container, false);
        Declaration_View(v);
        return v;
    }
    ...
    private void Declaration_View(View v) {
        ...
        if (getArguments() != null) {
            ...
            Select_Poll = getArguments().getString("Select_Poll");
    ...
    public void GetDataFromPref() {
        ...
        System.out.println("SELECTED_VALUES" + selected_value);
        ArrayList al = null;
        if (!(Common_Class.isRatefinderEmpty(PortPojoList))) {
            for (int i = 0; i < PortPojoList.size(); i++) {

                if ((PortPojoList.get(i).getPOL())
                        .equals(Select_Poll)) {

                    if (!(maker_list.contains(PortPojoList.get(i).getFINALPOD()))) {
                        maker_list.add(PortPojoList.get(i).getFINALPOD());
                        al = new ArrayList();
                        al.add(PortPojoList.get(i).getFINALPOD());
                        al.add(PortPojoList.get(i).getCountryCode());
                        System.out.println("+++params_data" + PortPojoList.get(i).getFINALPOD());
                    }
            ...
            for (Object o : al) {
                //do something o;
                System.out.println("NON GENRIC" + o);
        ...
        if (maker_list != null && maker_list.size() > 0) {
            ...
            makerAdapter = new POD_Adapter(maker_list, getActivity(), "POD", selected_value);
            mRecyclerview.setLayoutManager(recylerViewLayoutManager);
            mRecyclerview.setAdapter(makerAdapter);


        } else {
            LisitngMessage(R.drawable.ic_empty_location, "Pod Is Not Available");
