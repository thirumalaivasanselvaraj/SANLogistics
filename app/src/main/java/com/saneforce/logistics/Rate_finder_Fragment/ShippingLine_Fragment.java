...
import com.saneforce.logistics.Adapter_Class.Shipping_Line_Adapter;
...
public class ShippingLine_Fragment extends Fragment implements View.OnClickListener {
    ...
    String fragname, selected_value, Select_Departure;
    Shipping_Line_Adapter makerAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.shippingline_fragment, container, false);
        Declaration_View(v);
        return v;
    }
    ...
    private void Declaration_View(View v) {
        ...
        titlename.setText("Select ShipingLine");
        ...
        if (getArguments() != null) {
            ...
            Select_Departure = getArguments().getString("Select_Departure");
    ...
    public void GetDataFromPref() {
        ...
        System.out.println("SELECTED_VALUES" + Select_Departure);
        if (!(Common_Class.isRatefinderEmpty(PortPojoList))) {
            for (int i = 0; i < PortPojoList.size(); i++) {

                if ((PortPojoList.get(i).getDEP())
                        .equals(Select_Departure)) {
                    if (!maker_list.contains(PortPojoList.get(i).getLINE())) {
                        maker_list.add(PortPojoList.get(i).getLINE());
                    }
        ...
        if (maker_list != null && maker_list.size() > 0) {
            ...
            makerAdapter = new Shipping_Line_Adapter(maker_list, getActivity(), "Shiping_Line", selected_value);
            mRecyclerview.setLayoutManager(recylerViewLayoutManager);
            mRecyclerview.setAdapter(makerAdapter);


        } else {
            LisitngMessage(R.drawable.ic_empty_location, "Line Is Not Available");
